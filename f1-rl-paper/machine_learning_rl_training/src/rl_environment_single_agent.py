...
class RaceSimulation(tf_agents.environments.py_environment.PyEnvironment):
    ...
    Description:
        ...
        Note: This environment is not batched. It takes an action for a single driver (chosen randomly). After
        simulating one lap it gives back a time step for the chosen driver with observation and reward.
    ...
    Reward:
        Lap time (method: __calculate_reward_laptime()): Reward for the lap time since the previous pit stop decision
        Position (method: __calculate_reward_position()): Reward for positions won/lost since the previous pit stop
            decision
        Final position (method: __calculate_reward_final_position()): Reward for the final position
    ...
    __slots__ = (
        # environment inputs
        "__vse_type",             # determines which VSE (virtual strategy engineer) type is used for the other drivers
                                  # "supervised" or "reinforcement" or "basestrategy" or "realstrategy"
        ...
        "__idx_driver",                  # index of the driver controlled by the agent
        "__average_laptimes_presim",     # is calculated for the driver controlled by the agent in a pre-simulation
        "__idx_driver_behind_prev_lap",  # index of the driver behind the agent's driver at the end of the previous lap
        # race object --------------------------------------------------------------------------------------------------
        "__race",                        # race object
        # preprocessor -------------------------------------------------------------------------------------------------
    ...
    def __init__(self,
        ...
        self.idx_driver_behind_prev_lap = None
        ...
        # __init__ 마지막 근처에 추가
        race_pars = self.pars_in.get("race_pars", {}) if self.pars_in is not None else {}

        self.track_name = (
            race_pars.get("track")
            or race_pars.get("race")
            or getattr(self.race, "track_name", None)
        )
        ...
        self.last_tyre_wear_delta = 0.0

        # 타이어 마모 추적용(랩간 Δ 계산)
        self._prev_tyre_age = None
        
        # episode summary용 누적값
    ...
    def __get_driver_initials(self): return self.__driver_initials
    def __set_driver_initials(self, x): self.__driver_initials = x
    driver_initials = property(__get_driver_initials, __set_driver_initials)

    def __get_vse_type(self): return self.__vse_type
    def __set_vse_type(self, x): self.__vse_type = x
    vse_type = property(__get_vse_type, __set_vse_type)
    ...
    def __get_idx_driver(self): return self.__idx_driver
    def __set_idx_driver(self, x): self.__idx_driver = x
    idx_driver = property(__get_idx_driver, __set_idx_driver)
    ...
    def __calculate_observation(self) -> np.ndarray:
        """
        ...
        position = self.race.positions[cur_lap, self.idx_driver]
        tire_age = self.race.drivers_list[self.idx_driver].car.tireset.age_degr
        cur_compound = self.race.drivers_list[self.idx_driver].car.tireset.compound
        team = self.race.drivers_list[self.idx_driver].car.manufacturer
        strategy_info = self.race.drivers_list[self.idx_driver].strategy_info

        # calculate used_2compounds ------------------------------------------------------------------------------------
        used_2compounds = True if len({x[1] for x in strategy_info}) > 1 else False

        # calculate fcy_stat_curlap ------------------------------------------------------------------------------------
        idx_act_phase_rel = self.race.fcy_handling["idxs_act_phase"][self.idx_driver]
        ...
        if fcy_type is not None:
            fcy_start_end_progs = self.race.fcy_handling["start_end_prog"][self.idx_driver]
        ...
        est_pos_loss = len([x for x in racetimes_tmp
                            if racetimes_tmp[self.idx_driver] < x <= (racetimes_tmp[self.idx_driver] + t_lost)])
        ...
        if positions[self.idx_driver] != 1:
            idx_driver_ahead = [idx for idx in range(len(positions))
                                if positions[idx] == (positions[self.idx_driver] - 1)][0]
            interval = racetimes_tmp[self.idx_driver] - racetimes_tmp[idx_driver_ahead]
        else:
            # no other driver ahead
            interval = np.inf

        if positions[self.idx_driver] == self.race.no_drivers:
            ahead = np.inf
        else:
            ...
            if not bool_driving[idx_driver_behind]:
                ...
                ahead = racetimes_tmp[idx_driver_behind] - racetimes_tmp[self.idx_driver]
        ...
        if self.idx_driver_behind_prev_lap in self.race.pit_driver_idxs:
            # gap between driver and driver behind minus the time a pit stop would cost
            gap_aft_stop = racetimes_tmp[self.idx_driver_behind_prev_lap] - racetimes_tmp[self.idx_driver] - t_lost
        ...
        if positions[self.idx_driver] == self.race.no_drivers:
            ...
            self.idx_driver_behind_prev_lap = next((idx for idx, pos in enumerate(positions)
                                                    if pos == positions[self.idx_driver] + 1), None)
        ...
        X_cat = np.zeros(7)
        X_cat[0] = self.cat_preprocessor.transform_cat_dict(X_cat_str=[cur_compound], featurename='cur_compound')[0]    # 7. Current Tyre
        X_cat[1] = used_2compounds      # ?. Whether more than two compounds are used
        X_cat[2] = fcy_stat_curlap      # 3. Safety Car
        X_cat[3] = close_behind
        X_cat[4] = close_ahead
        X_cat[5] = defendable_undercut
        X_cat[6] = self.cat_preprocessor.transform_cat_dict(X_cat_str=[self.driver_initials],
    ...
    def __calculate_reward_laptime(self, t_pitdrive_inlap: float) -> float:
        ...
        cur_laptime = float(self.race.laptimes[cur_lap, self.idx_driver]) + t_pitdrive_inlap
        ...
        if fcy_type is None:
            ...
            reward = self.average_laptimes_presim[self.idx_driver] - cur_laptime
        else:
            fcy_start_end_progs = self.race.fcy_handling["start_end_prog"][self.idx_driver]
            ...
            average_laptime = (delta * self.race.track.t_lap_fcy
                               + (1.0 - delta) * self.average_laptimes_presim[self.idx_driver])
            reward = round(average_laptime - cur_laptime, 6)

            if fcy_type == 'SC':
                racetime_tmp = (self.race.racetimes[cur_lap - 1, self.idx_driver]
                                + self.race.laptimes[cur_lap, self.idx_driver])

                sc_ghost_incl_min_dist = (self.race.fcy_handling["sc_ghost_racetimes"][self.idx_driver]
                                          + self.race.positions[cur_lap, self.idx_driver]
    ...
    def _reset(self) -> tf_agents.trajectories.time_step.TimeStep:
        ...
        # 공개 상태 초기화 (아직 끝난 랩 없음)
        self.current_lap = None
        self.position_prev = int(self.race.positions[self.race.cur_lap, self.idx_driver])
        self.position = None
        self.last_lap_time = None
        self.last_pit_time_loss = 0.0
        self._prev_tyre_age = float(self.race.drivers_list[self.idx_driver].car.tireset.age_degr)
        self.last_tyre_wear_delta = 0.0
        
        # episode summary 누적값 초기화
        self._episode_length = 0
        self._pit_count = 0
        self._lap_time_sum = 0.0
        self._lap_count = 0
        ...
        # get the observation after the first lap and before first pit stop decision
        observation = self.__calculate_observation()

        return tf_agents.trajectories.time_step.restart(observation=observation)

    def _step(self, action) -> tf_agents.trajectories.time_step.TimeStep:
        """
        ...
        # --- 시뮬 전: 랩 시작 시점 정보(포지션, 타이어 연식 등) 저장 ---
        position_before_simulate_lap = int(self.race.positions[self.race.cur_lap, self.idx_driver])
        laptime_before_pitstop = float(self.race.laptimes[self.race.cur_lap, self.idx_driver])
        tyre_age_before = float(self.race.drivers_list[self.idx_driver].car.tireset.age_degr)

        if action in [1, 2, 3]:
            # pit stop with chosen compound
            self.race.drivers_list[self.idx_driver].strategy_info.append(
                [self.race.cur_lap, self.available_compounds[action - 1], 0, 0.0]
            )

        # --- 한 랩 시뮬레이션 ---
        self.race._RaceReinftrain__simulate_lap()

        # 이 시점에 cur_lap가 다음 랩으로 넘어갔으므로,
        # "방금 끝난 랩"의 인덱스는 (self.race.cur_lap - 1)
        finished_lap_idx = int(self.race.cur_lap - 1)

        # 랩 종료 포지션(방금 끝난 랩의 포지션)
        position_after_simulate_lap = int(self.race.positions[finished_lap_idx, self.idx_driver])
        delta_position = position_before_simulate_lap - position_after_simulate_lap

        # 피트 손실: 이 랩에서의 추가 손실(기존 계산 유지)
        laptime_after_pitstop = float(self.race.laptimes[finished_lap_idx, self.idx_driver])
        t_pitdrive_inlap = laptime_after_pitstop - laptime_before_pitstop

        # --- 공개 상태 필드 업데이트(랩 경계 정보) ---
        self.current_lap = finished_lap_idx
        self.position_prev = position_before_simulate_lap
        self.position = position_after_simulate_lap
        self.last_lap_time = laptime_after_pitstop
        self.last_pit_time_loss = max(float(t_pitdrive_inlap), 0.0)

        # ===== episode summary 누적 =====
        self._episode_length += 1

        # action이 pit이면 pit_count 증가 (가장 단순/명확)
        if action in [1, 2, 3]:
            self._pit_count += 1

        # lap time 평균용 누적
        self._lap_time_sum += float(laptime_after_pitstop)
        self._lap_count += 1

        # 타이어 마모 Δ (가능하면: 현재 age_degr - 이전 age_degr)
        tyre_age_after = float(self.race.drivers_list[self.idx_driver].car.tireset.age_degr)
        if self._prev_tyre_age is None:
            self.last_tyre_wear_delta = 0.0
        else:
            self.last_tyre_wear_delta = max(0.0, tyre_age_after - self._prev_tyre_age)
        self._prev_tyre_age = tyre_age_after

        # --- 관측/보상 계산(기존 로직 유지) ---
        observation = self.__calculate_observation()

        reward = 0.1 * (
            self.__calculate_reward_laptime(t_pitdrive_inlap) +
            self.__calculate_reward_position(np.int32(delta_position))
        )
        ...
        if self._episode_ended:
            ...
            last_lap = self.race.get_last_compl_lap(idx=self.idx_driver)
            self._final_position = int(self.race.positions[last_lap, self.idx_driver])
            
            reward += self.__calculate_reward_final_position()
            used_2compounds = (True if len({x[1] for x in self.race.drivers_list[self.idx_driver].strategy_info}) > 1 else False)
            ...
            return tf_agents.trajectories.time_step.termination(observation=observation, reward=reward)
        else:
            return tf_agents.trajectories.time_step.transition(observation=observation, reward=reward, discount=1.0)
    ...
    def get_state(self):
        return {
            ...
            "last_pit_time_loss": self.last_pit_time_loss,
            "last_tyre_wear_delta": self.last_tyre_wear_delta,
        }

    def get_episode_summary(self):
        avg_lap_time = None
        if self._lap_count > 0:
            avg_lap_time = self._lap_time_sum / float(self._lap_count)

        return {
            "track": getattr(self, "track_name", None),
            "season": None,   # 너 wrapper/meta에서 넣고 있으면 거기서도 OK
            "round": None,
            "seed": self.seed_value,   # 숫자로만!
            "episode_length": int(self._episode_length),
            "pit_count": int(self._pit_count),
            "avg_lap_time": float(avg_lap_time) if avg_lap_time is not None else None,
            "final_position": int(self._final_position) if self._final_position is not None else None,
        }
